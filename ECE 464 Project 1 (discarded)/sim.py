...
from faultSimHelper import faultAnalyze
from faultSimHelper import indexSwap
...
def main():
    ...
    faultOut=open("fault_sim_result.txt","w+")
    ...
    for line in inputFile:
        ...
        # print("\n FAULT INSERTION ----------------------------")
        # gateOut="wire_fault"
        # circuit[gateOut] = ['NOT', ['wire_a'], True, '1']
        # print("\n DICT HERE ----------------------------")
        # for cKeys, value in sorted(circuit.items()):
        #     print (cKeys, value)
        # print("\n DICT END ----------------------------")
        ...
        print("\n ***********************NORMAL OPERATION********************")
        circuit=basic_sim(circuit) #Normal Circuit Operation
        goodOut=circuit[circuit['OUTPUTS'][1][0]][-1]
        print("tv"+str(lnCounter)+" = " + line + " -> " + goodOut + " (good)")
        faultOut.write("\n"+"tv"+str(lnCounter)+" = " + line + " -> " + goodOut + " (good)")

        fc=0 #counter for iterating through fault list (since faultCounter isn't an int)
        for faultCounter in faults:
            #printCkt(circuit)
            #print("CIRCUIT OUTPUT"+ str(circuit["OUTPUTS"][1][-1]))
            print("\n **********************INSERTING FAULT********************")
            # #faultData=faultAnalyze(faults,faultCounter)
            faultData=faultAnalyze(faults,fc)

            if faults[fc][1][1]=='SA':
                circuit[faultData[1]][-1]=faultData[0] #Assign wire to SA fault
            else:
                deadWire="wire_fault"
                circuit[deadWire] = ['NOT', ['wire_fault'], True, faultData[0]] #create temp wire with stuck signal

                print("\n -------------------------INCOMING FAULT DATA-----------------------------------------")
                print(faultData[1])
                print(faultData[2])
                print(circuit[faultData[1]])
                print(circuit[faultData[2]])


                faultSwap=indexSwap(circuit[faultData[1]], faultData[2]) #index at which we need to swap X-IN-Y with SA
                circuit[faultData[1][faultSwap]]=circuit[deadWire]

            circuit=basic_sim(circuit)
            faultyOut=circuit[circuit['OUTPUTS'][1][0]][-1]
            print("tv"+str(lnCounter)+" = " + line + " -> " + faultyOut + " (faultyOut)")
            faultOut.write("\n"+"tv"+str(lnCounter)+ " " + faults[fc][1][1] + " = " + line + " -> " + faultyOut + " (faultyOut)")


            print("\n **********************OUTPUT ARRAY********************")


            #print("OUTPUT= " + circuit[circuit['OUTPUTS'][1][0]][-1])  #WORKS
            ...
            # print(circuit['wire_C'])
            # print(circuit['INPUTS'])
            # print("/n -------------IDX--------------")
            # print(circuit['wire_C'][3])
            # print("/n --------------BREAK-----------")
            # print(circuit['wire_C'][-1])
            # print("/n --------------LAST ELEMENT FOUND-----------")

            # print(circuit['OUTPUTS'])
            # print(circuit['OUTPUTS'][1])
            # print(circuit['GATES'])
            # print(circuit['INPUT_WIDTH'])
